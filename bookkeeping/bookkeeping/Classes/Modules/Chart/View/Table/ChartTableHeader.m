...
- (void)setModel:(BookChartModel *)model {
    _model = model;
    _chart.model = model;
    _avgLab.text = [NSString stringWithFormat:KKLocalized(@"平均值: %0.2f"), model.avg];
    _maxLab.text = ({
        NSString *str;
        if (model.is_income == false) {
            str = [NSString stringWithFormat:KKLocalized(@"总支出: %0.2f"), model.sum];
        } else {
            str = [NSString stringWithFormat:KKLocalized(@"总收入: %0.2f"), model.sum];
        }
        str;
    });
}

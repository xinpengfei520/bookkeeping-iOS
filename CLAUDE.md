...
- **Networking.** Always go through `AFNManager POST:params:complete:` (or `GET:params:complete:` for query-string endpoints such as `/book/rates`). It auto-injects `Authorization`, persists a refreshed token from the response header *and* from the standard `token` / `expiresIn` body fields via `UserInfo`, and posts `MINE_TOKEN_EXPIRED` on `result.code == TOKEN_EXPIRED`. Endpoints belong in `NSString+API.h` as `Request(@"/path")` macros.
- **Auth & session.** `UserInfo` (singleton) holds login state and the token. **Storage is the bare JWT** — `saveAuthorizationToken:` strips any `Bearer ` prefix (legacy stored values are stripped on read too, so upgrades aren't logged out) and `getAuthorizationHeader` is the only thing that prepends `Bearer `; never concatenate the prefix at a call site or you get the double-prefix the server rejects. Callers gate features with `[UserInfo isLogin]` and present `LoginController` when false. Face ID gating on app launch is keyed off the `PIN_SETTING_FACE_ID` `NSUserDefaults` flag and routed through `LAContextManager`.

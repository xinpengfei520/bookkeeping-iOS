...
- ✅ Pod 依赖从 19 个降到 9 个；第三方源码净减 ~160K LoC；BaseViewController 从 152 行收敛到 ~40 行
- ✅ 设计/计划文档落在 `docs/superpowers/specs/` 与 `docs/superpowers/plans/`

#### 待办
- [ ] **ReactiveObjC 替换为系统 API**（约 26 文件）：项目主要用 `@weakify`/`@strongify` 防循环引用 + `rac_addObserverForName:` 包装 NSNotificationCenter + `rac_signalForControlEvents:` 包装按钮事件。三类用法都有直接的系统替代（`__weak typeof(self) weakSelf = self;` 宏 / NSNotificationCenter 直接用 / addTarget:action: 直接用）。ReactiveObjC 自 2018 年起维护停滞。预计 2 天工作量。
- [ ] **MJRefresh 替换为 UIRefreshControl + 自写 footer**（约 14 文件）：下拉刷新可换成 iOS 6+ 标配 `UIRefreshControl`；上拉加载系统无对应 API，需自写一个 ~50 行的 footer view。预计 1 天工作量。

...
### 变更
- **首页记账条**：默认只显示圆形 +。点按记一笔；长按展开语音 / 图片，左滑或右滑松手进入对应入口，松手后两侧收回。

### 新增
- **相册 OCR 记账（M3）**：长按 + 号左滑松手打开相册（最多 9 张），端上 Vision 识别账单文字后拆成 1~N 笔，确认列表可点改 / 左滑删除，一并走现有落库管线。单笔且规则没补全时仍复用 `POST /book/parse`。模拟器可用 `KK_DEBUG_OPEN=ocr`。
  - 已知：规则拆条准确度低，后续改 Dify + 大模型，见 `docs/语音记账方案设计.md` 第七节。

---

## [1.0.20] (build 21) — 2026-08-17

### 新增
- **多币种扩到 8 个外币**：汇率页与记账币种选择器按 `GET /book/rates` 返回的键渲染，不再写死 USD / HKD / SGD；新增日元 JPY、韩元 KRW、英镑 GBP、欧元 EUR、加拿大元 CAD。后续接口再加币种无需跟版。
- **语音记账 M2 LLM 兜底**：规则解析失败时走 `POST /book/parse`（`KKLLMParser`），确认卡片可继续编辑后再落库。

### 变更
- 录音浮层改为全屏磨砂背景，和底部账单列表分开。
- API Host 改由 xcconfig 注入，生产 / 测试切换不用改代码。
...
### 修复
- **换账号数据串号**：退出登录 / 删除账号时漏清离线队列与备注列表 —— A 账号未补发的离线记账会被补发到 B 账号名下，推荐备注也跨账号泄露。
- **删除失败记录会"复活"**：旧实现只在成功分支删本地，弱网下界面已移除、SQLite 还留着，切月份回来记录重现。现改为本地立即删 + 失败入队补发。
- **修改失败重启后回滚**：同理改为本地立即写入 + 失败入队补发；业务拒绝时明确提示未同步。

### 变更
- 离线队列升级为**带操作类型的待办队列**（`KKPendingBookOp`：add/update/delete），入队按 bookId 合并：排队新增再改仍是新增（临时 id 绝不发 update）、排队新增再删则整条撤销、排队修改再删变为删除。自动兼容 1.0.16 及更早的旧队列格式。
- 本地临时 `bookId` 改为**负数区间**，与服务端 id（恒为正）彻底隔离，消除主键碰撞隐患。
- Siri「记一笔」在后台拉起 App 进程时，首页不再触发 Face ID 验证与网络同步，推迟到真正进前台。

## [1.0.16] (build 17) — 2026-08-05

### 新增
- **汇率 7 天走势**：今日汇率页新增走势卡（sparkline，标注区间高/低与首尾日期），点击币种行切换；7 天数据并发拉取、切换零请求。
- **Siri 捷径**：
  - 对话式「记一笔」（AppIntents）：对 Siri 说"用记呀记一笔"，报金额即可，不打开 App；本地落库 + 下次打开自动同步服务端。
  - 设置 → Siri 捷径（仿网易云交互）：为"打开记账键盘 / 看今日汇率 / 看图表账单"录制自定义唤醒口令，可随时编辑口令或移除。
